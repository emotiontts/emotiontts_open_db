...
def _text_to_sequence(text, cleaner_names, as_token, _symbol_to_id):
    ...
    while len(text):
        ...
        if not m:
            ...
            # sequence += _symbols_to_sequence(text, _symbol_to_id)
            break
        sequence += _symbols_to_sequence(
            _clean_text(m.group(1), cleaner_names), _symbol_to_id
        )
...
def _symbols_to_sequence(symbolss, _symbol_to_id):
    # _symbol_to_id = {s: i for i, s in enumerate(symbols)}
    # print([_symbol_to_id[s] for s in symbolss if _should_keep_symbol(s, _symbol_to_id)])
    symbols_tmp = []
    for s in symbolss:
        if s == " ":
            symbols_tmp.append("<space>")
        else:
            symbols_tmp.append(s)
    symbolss = symbols_tmp
    return [_symbol_to_id[s] for s in symbolss if _should_keep_symbol(s, _symbol_to_id)]
...
def _should_keep_symbol(s, _symbol_to_id):
    # _symbol_to_id = {s: i for i, s in enumerate(symbols)}
    # print((s in _symbol_to_id and s is not '_' and s is not '~'))  # or s is ' ')
