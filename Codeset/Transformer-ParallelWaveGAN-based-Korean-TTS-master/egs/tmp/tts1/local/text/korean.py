...
count_checker = (
    "(시|명|가지|살|마리|포기|송이|수|톨|통|개|벌|척|채|다발|그루|자루|줄|켤레|그릇|잔|마디|상자|사람|곡|병|판|군데|곳|달)"
)
...
def normalize_number(text):
    ...
    text = re.sub(
        number_checker + noncount_checker, lambda x: number_to_korean(x, True), text
    )
    text = re.sub(
    ...
    if not is_count:
        ...
        if any(word in kor for word in noncount_tenth_dict):
            kor = re.sub(
                ...
                lambda x: noncount_tenth_dict[x.group()],
                kor,
            )
